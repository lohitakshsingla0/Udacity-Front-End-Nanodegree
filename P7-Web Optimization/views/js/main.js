...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
	  ...
	  var dx = determineDx(PC[0], size);
	  var newwidth = (PC[0].offsetWidth + dx) + 'px';
...
function updatePositions() {
  var para = [];
    ...
    for (var q = 0; q < 5; q++) {
        para.push(Math.sin(document.body.scrollTop/1250 + q ));
    }
    

    for (var i = 0, max=items.length;i < max; i++) {
        items[i].style.left = items[i].basicLeft + para[i%5] * 100 + 'px';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for(var q =32;q >0;q--) {
    ...
    elem.style.width = "70.000px";
    elem.basicLeft = (q % cols)*s;
    elem.style.top = (Math.floor(q/cols)*s)+'px';
